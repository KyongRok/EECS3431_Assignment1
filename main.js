...
var bubbleBurst = Math.round(Math.random())+5; //random burst number
var lastPos = []; //saves first position of every bubble
var lastTime = []; //saves first TIME of each bubble
var currBubble = 0; //current frame
var bubbleCount = 0; //number of bubbles
var bubbleShootTimer = 0; //times for when the bubble should appear
...
function render() {
    ...
    gPush();
    {
        ...
        let didNewBubble = false; //checks if a new bubble was made
        for(let i = 0; i <= currBubble; i++){
            if(lastPos.length == i && TIME > bubbleShootTimer){
                lastPos.push(pos); //add new bubble location
                lastTime.push(TIME); //add time of creation
                didNewBubble = true; //a new bubble was found
            }
            createBubble(i);
        }
        currBubble++;
        ...
        if(didNewBubble){
            bubbleShootTimer = TIME + 0.5; //change this to change interval between each bubble
            bubbleCount++;
            if(bubbleCount >= bubbleBurst){
                bubbleShootTimer = TIME + 5; //change this to change interval between each bubble burst
                bubbleBurst += Math.round(Math.random())+5;
            }
        }
    }
    gPop();
